...
def point_ended(bound_crossed):
    ...
    turns.append(turn(curves, arr2, arr))
    bounceLocs.clear()
...
while True:
    ...
    for b in bounces_2d:
        tempx, tempy = b
        cv2.circle(img2, (tempx, tempy), 3, (0, 0, 255), 2)
        ...
        for i in circles[0]:
            ...
            if(loc.x != None):
                ...
                if(len(ballLocations1) >= 2):
                    if(ballLocations1[-1].side != ballLocations1[-2].side):
                        bounces_on_current_side = 0
    ...
    if(DISPLAY_FPS):
        ...
        avg = str(avg)
        cv2.putText(img, avg, (10, 50), cv2.FONT_HERSHEY_PLAIN, 3, (0, 0, 0), 3)
